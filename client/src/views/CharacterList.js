...
const CharacterList = () => {
  ...
  const handleSearch = async () => {
    try {
      ...
      await handleFilmInfo(response.data.results[0].url)
      await handleVehicleInfo(response.data.results[0].url)
    } catch (error) {
      console.error('Error searching characters:', error);
    }
  };

  const handleFilmInfo = async (url) => {
    //retrieving all the film info 
    try {
      let movies = []
      const response = await axios.get(url);
      response.data.films.map((film) => {
         axios.get(film).then((data)=>{movies.push(data.data.title)});
      }
      )
      setAllmovies(movies)
      //console.log(movies)  
  ...
  const handleVehicleInfo = async (url) => {
    // retreiving all the vehicle info
    try {
      let vehicless = []
      const response = await axios.get(url);
      response.data.vehicles.map((vehicle) => {
         axios.get(vehicle).then((data)=>{vehicless.push(data.data.name)});
      }
      )
      setAllVehicles(vehicless)
  ...
  const getCompleteList = async () => {
    try {
      ...
      for(let i = 1; i < 10; i++){
        ...
        response.data.results.forEach((data)=>{charsArr.push(data)})
      }
  ...
  return (
    <div style = {{marginTop:'10%'}}  >
          ...
          <TableContainer component={Paper} style={{marginTop:'5%', marginBottom:'2%'}}>
